...
- **全新侧边栏布局**: 深色渐变背景，10 个导航入口，支持折叠/展开
- **响应式适配**: 移动端侧边栏覆盖层，自动检测屏幕宽度
- **独立布局**: 管理后台不再显示前台 AppHeader/MobileNav/CreateProjectFab
- **面包屑导航**: 自动显示当前位置（管理后台 / 当前页面）
- **路由过渡动画**: 页面切换 fade-slide 动画效果

### 👥 后台用户管理 (AdminUsersView)

- 用户列表（分页、搜索用户名/昵称、角色筛选、状态筛选）
- 编辑用户（修改角色 STUDENT/TEACHER/ADMIN、修改状态）
- 禁用/启用用户（含防自禁用保护）
- 批量操作（批量禁用、批量启用）
- 用户详情弹窗（完整信息展示）
- 导出 CSV（含 BOM 头，Excel 兼容）
- 统计条（总数/正常/禁用）

### 🎮 后台作品管理 (AdminProjectsView)

- 作品列表（分页、搜索标题/作者、状态筛选）
- 统计卡片（总作品/已发布/草稿/审核中）
- 状态变更（发布/下线/设为审核中，下拉菜单操作）
- 删除作品（逻辑删除，二次确认）
- 预览作品（新窗口打开前台详情页）
- 封面缩略图 + 作者信息 + 互动数据展示

### 💬 后台评论管理 (AdminCommentsView)

- 评论列表（分页、搜索评论内容）
- 删除评论（管理员权限，同步减少项目评论数）
- 查看作品（跳转到前台项目详情）
- 评论详情展示（用户信息 + 评论内容 + 关联作品）

### 🏆 后台竞赛管理 (AdminCompetitionsView)

- 竞赛列表（分页、类型标签、状态标签）
- 查看竞赛详情（跳转前台竞赛详情页）

### 📚 后台班级管理 (AdminClassesView)

- 班级列表（分页、教师信息、学生数、邀请码）

### 📈 后台数据统计 (AdminStatisticsView)

- 核心指标卡片（用户/项目/题目/提交/班级/作业）
- 用户角色分布图（学生/教师/管理员）
- 项目状态分布图（已发布/草稿）
- 判题通过率图表（AC/其他）
- 关键比率进度条（发布率/通过率/增长率）
...
- `GET /api/v1/admin/project` — 作品列表（支持 keyword/status 筛选）
- `GET /api/v1/admin/project/stats` — 作品统计
- `PUT /api/v1/admin/project/{id}/status` — 更新作品状态
- `DELETE /api/v1/admin/project/{id}` — 删除作品（逻辑删除）
- `GET /api/v1/admin/comment` — 评论列表（支持 keyword 搜索）
- `DELETE /api/v1/admin/comment/{id}` — 删除评论
- `PUT /api/v1/admin/user/{id}` — 更新用户角色/状态

### 🎨 前端架构改进

- **路由重构**: 管理模块使用 AdminLayout 嵌套子路由（10 个子路由）
- **App.vue**: 条件渲染 — 管理后台路由隐藏前台导航组件
- **admin.ts API**: 新增作品管理/评论管理/用户更新接口
- **AdminView 简化**: 移除内联用户管理表格，改为快速导航入口

### 📝 文档更新

- TODO.md: 新增 Phase 15 记录
- CHANGELOG.md: v3.8.0 完整变更日志

---

## [v3.7.0] - 2026-04-30 — 前端大规模增强与 Scratch 全面集成

### 🎨 前端新增页面

- **ClassManageView**: 班级管理（创建/加入/成员管理/邀请码复制/退出班级）
- **AuditView**: 内容审核（敏感词审核/通过/拒绝/筛选/统计）
- **SystemConfigView**: 系统配置管理（查看/编辑/添加配置项）
- **ProblemManageView**: 题目管理（创建/发布/删除，支持选择题/判断题/编程题）
- **HomeworkCreateView**: 作业布置（选班级/关联题目/设置截止时间/发布/关闭）

### 🎨 前端页面增强

- **UserProfileView**: 全面重写
  - Hero 风格头部（渐变背景 + 头像等级徽章）
  - 关注/取消关注功能（对接后端 API）
  - 统计数据栏（作品/获赞/浏览/积分）
  - Tab 切换（作品列表/数据概览）
- **HomeworkDetailView**: 全面重写
  - 教师视角：提交列表 + 批改弹窗 + 发布/关闭操作
  - 学生视角：提交作业 + 查看批改结果 + 创建作品引导
  - 提交进度条和统计信息
- **CompetitionDetailView**: 全面重写
  - 实时倒计时和进度条
  - 金银铜牌排名展示
  - 报名/进入答题按钮
- **SettingsView**: 增强
  ...
  - 外观设置（深色模式/语言切换）
  - 第三方账号绑定区域
- **AdminView**: 快速导航扩展至 12 个入口
- **FeedView**: 新增「创建作品」按钮

### 🎮 Scratch 编辑器/预览集成

- **ScratchPreview 组件**: 可复用 Scratch 项目预览
  - 支持 small/medium/large 三种尺寸
  - 自动加载/手动加载模式
  - 全屏/编辑/重载控制按钮
  - TurboWarp 嵌入播放器
- **ScratchPreviewDialog**: 快速预览弹窗
  - 项目卡片 ▶️ 按钮触发
  - 全屏预览 + 跳转详情/编辑器
- **CreateProjectFab**: 全局浮动创建按钮
  - 固定在右下角，脉冲动画
  - 展开菜单：新建空白/导入 SB3/进入编辑器
- **ProjectCard**: 新增快速预览按钮（hover 显示 ▶️）
- **ProjectDetailView**: 嵌入 Scratch 实时播放器

### 🎭 UI/UX 优化

- 全局 CSS 动画系统（20+ 微交互动画）
  - 卡片入场动画（card-enter，带交错延迟）
  - 数字跳动效果（number-pop）
  - 浮动动画（float）/ 脉冲发光（pulse-glow）
  - 标题 emoji 波浪动画
  - 骨架屏闪烁效果 / 按钮点击波纹
  - 进度条光泽动画 / 表格行悬停增强
  - Tag 微交互缩放 / 头像悬停旋转
  ...
  - Tab 下划线动画 / 项目卡片悬停增强
- 导航栏新增「班级」入口
- i18n 新增 `nav.class` 中英文翻译

### 📊 测试

- 前端单元测试：166/166 ✅
- 后端集成测试：20/20 ✅
- 前端构建：899KB (gzip 287KB)
