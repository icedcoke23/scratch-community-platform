...
CREATE INDEX IF NOT EXISTS `idx_submission_user_problem_time`
    ON `submission` (`user_id`, `problem_id`, `created_at` DESC);

-- 2. project_like: 按时间排序的点赞列表 (project_id + created_at DESC)
CREATE INDEX IF NOT EXISTS `idx_project_like_project_time`
